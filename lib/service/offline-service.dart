...
class OfflineService {
  ...
  void init() {
    groups ??= StreamController.broadcast();
    tags ??= StreamController.broadcast();
    barcodes ??= StreamController.broadcast();
  }
  ...
  List<T> _getFromLocal<T>(String key, T Function(dynamic) factory) {
    ...
    if (jsonStr != null) {
      ...
      return list.map(factory).toList();
    }
    return [];
  }
  ...
  List<BarcodeGroup> _getGroupsFromLocal() => _getFromLocal<BarcodeGroup>(
      LOCALSTORE_GROUPS, (map) => BarcodeGroup.fromJson(map as Map<String, dynamic>));

  Future<void> _saveGroupsToLocal(List<BarcodeGroup> groups) =>
      _saveToLocal<BarcodeGroup>(LOCALSTORE_GROUPS, groups, (g) => g.toJson());

  List<Tag> _getTagsFromLocal() => _getFromLocal<Tag>(
      LOCALSTORE_TAGS, (map) => Tag.fromJson(map as Map<String, dynamic>));
  ...
  List<Barcode> _getBarcodesFromLocal() => _getFromLocal<Barcode>(
      LOCALSTORE_BARCODES, (map) => Barcode.fromJson(map as Map<String, dynamic>));
  ...
  Stream<Barcode?> streamBarcode(String uid) {
    final sc = StreamController<Barcode?>.broadcast();
    Future.delayed(const Duration(milliseconds: 100)).then((_) {
      if (sc.isClosed) return;
      final barcodeList = _getBarcodesFromLocal();
      try {
        sc.add(barcodeList.firstWhere((b) => b.uid == uid));
      } catch (_) {
        sc.add(null);
      }
    });
  ...
  Stream<List<Barcode>> streamBarcodesByGroup(String groupId) {
    final sc = StreamController<List<Barcode>>.broadcast();
    Future.delayed(const Duration(milliseconds: 100)).then((_) {
      if (sc.isClosed) return;
      final list = _getBarcodesFromLocal()
